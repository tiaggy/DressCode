...
public class SkinUtils {
	private final static String pluginFolder = DressCodePlugin.getPlugin().getDataFolder().getAbsolutePath();
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if (playerData.getBasicSkin().equals("0")) {
					...
					try {
						Files.deleteIfExists(originalSkin.toPath());
						Files.deleteIfExists(skin.toPath());
						updateSkin(player, playerData);
					} catch (IOException | InterruptedException e) {
						throw new RuntimeException(e);
					}
	...
	public static void deleteSkinData(String keyString) {
		...
		if (files != null) {
			for (File file : files) {
				if (file.isFile()) {
					...
					if (!deleted) {
						getPluginLogger().warning("Failed to delete: " + file.getName());
					} else {
						filesDeleted++;
					}
				}
		...
		getPluginLogger().info("Deleted " + filesDeleted + " skin data files for: " + keyString);
	}
	...
	private static void downloadDefaultSkin(Player player) throws IOException, InterruptedException {
		...
		if (skinUrl != null) {
			...
			if (!playerSkinFolder.exists()) {
				Files.createDirectories(playerSkinFolder.toPath());
			}
		...
		if (defaultSkin == null) {
			...
			while (texture == null) {
				try {
					...
					getPluginLogger().warning("Failed to get skin data for: " + skinFileName);
				}
			}
			...
			if (playerData.equals(DressCodePlugin.getPlayerAppearance(player))) {
				try {
					updateSkin(player, playerData);
				} catch (IOException | InterruptedException e) {
					throw new RuntimeException(e);
			...
			}.runTask(DressCodePlugin.getPlugin());
	...
	private static String getSkinUrl(String uuid) throws IOException {
		...
		if (connection.getResponseCode() == 200) {
			JsonObject json = parseResponce(connection);
	...
	private static JsonObject parseResponce(HttpURLConnection connection) throws IOException {
	...
	private static final String DEFAULT_BOTTOM_REGEX = DressCodePlugin.getPlugin().getConfig().getString("nsfw-censorship.bottom_default_keyword");
	private static final String DEFAULT_TOP_REGEX = DressCodePlugin.getPlugin().getConfig().getString("nsfw-censorship.top_default_keyword");
		...
		if (!f.exists()) {
			...
			if (customSkinExists(basicSkin)) {
				...
				if (fileName.contains("#0#0#0#0#0")) {
					assert DEFAULT_TOP_REGEX != null;
					if (!basicSkin.contains(DEFAULT_TOP_REGEX)) {
						assert DEFAULT_BOTTOM_REGEX != null;
						if (!basicSkin.contains(DEFAULT_BOTTOM_REGEX)) {
	...
	private static void saveSkinData(String fileName, JsonObject skinData) {
		...
		try (FileWriter fileWriter = new FileWriter(skinDataFolder + fileName + ".json")) {
			...
			getPluginLogger().severe("Failed to save skin data: " + e.getMessage());
		}
	}
}
